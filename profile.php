...
{
    if(!empty($_POST['btnAction']) && $_POST['btnAction'] =='Follow')
    {
        addFollowsFeature($_SESSION['id'], $_POST['friend_to_follow']);
    }
    else if(!empty($_POST['btnAction']) && $_POST['btnAction'] =='Unfollow')
    {
        unfollowsFeature($_SESSION['id'], $_POST['friend_to_unfollow']);
    }
...
<?php
    $getAllOtherUsers = getOtherUsers($_SESSION['id']);
    ?>
...
<?php foreach ($getAllOtherUsers as $friend_info): ?>
  <tr>
     <td><?php echo $friend_info["username"]; ?></td>
